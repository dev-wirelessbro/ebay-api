...
class EbayClient {
  ...
  run(actionName, options, parseOptions, extraHeaders) {
    return this.request(actionName, options, null, parseOptions, extraHeaders).then(
      result => {
        const pageNumberParser = EbayClient.pageNumberParsers[actionName] ||
          (result => ({
            pageNumber: parseFloat(result.PageNumber || 0),
            totalPage: parseFloat(_.get(result, "PaginationResult.TotalNumberOfPages", 0))
          }));

        const {
          pageNumber,
          totalPage
        } = pageNumberParser(result)

        if (pageNumber === totalPage) {
          return result;
        } else {
          return Promise.mapSeries(_.range(2, totalPage + 1), pn => {
              return this.request(actionName, options, {
                page: pn,
...
verbs.forEach(verb => {
  ...
  EbayClient.prototype[action] = function (options = {}, parseOptions = {}, extraHeaders) {
...
EbayClient.pageNumberParsers = {
  GetMyeBaySelling: result => {
    ...
    if (_.isNaN(totalPage)) {
      totalPage = parseFloat(_.get(result, "ActiveList.PaginationResult.TotalNumberOfPages"))
    }
