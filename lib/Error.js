    ...
    if (Array.isArray(json.Errors)) {
      errorText = json.Errors.filter(error => error.SeverityCode === 'Error').map(error => {
        if (error.ErrorCode == "240") {
          errorParam = error.ErrorParameters instanceof Array ? error.ErrorParameters[0].Value : error.ErrorParameters.Value
        }
        return error.LongMessage
      }).join();
    } else {
      errorText = json.Errors.LongMessage;
      if (json.Errors.ErrorCode == "240") {
        errorParam = json.Errors.ErrorParameters instanceof Array ? json.Errors.ErrorParameters[0].Value : json.Errors.ErrorParameters.Value
      }
    }
    const errRtn = {
      errMsg: `The reply from ebay is failure ${errorText}`,
      errParams: errorParam
    }
    super(`${JSON.stringify(errRtn)}`);
  }
};
